...
def send_mailTrap_email(sender: str, reciever: str, subject: str, markdown_path: str):
  ...
  with smtplib.SMTP("sandbox.smtp.mailtrap.io", int(os.getenv("FAKE_SMTP_PORT"))) as server:
      server.login(os.getenv("FAKE_SMTP_LOGIN_USER"), os.getenv("FAKE_SMTPLOGIN_PASS"))
...
def main():
    ...
    send_email("mailtrap@email.com","ska46@njit.edu","subject","emails/markdown0.md")
    #send email2 to mailtrap inbox using faker
    fake = Faker()
    #send_email(fake.email(),fake.email(),fake.sentence(),"emails/markdown1.md")
